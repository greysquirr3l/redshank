...
| T51 — credential setup helper: guided onboarding tool for all required API keys | `[ ]` | Redshank requires credentials for ~40 data sources across campaign finance, OSINT, courts, sanctions, and corporate registries — a significant operator burden. Build a `redshank-setup` crate (or standalone binary sub-command within `redshank-cli`) that walks operators interactively through each credential, validates it with a live probe, and writes it to `credentials.json` at the correct path. Key decisions: (1) standalone crate vs. `redshank setup` subcommand; (2) TUI wizard (ratatui) vs. plain CLI prompts; (3) grouping by datasource category; (4) optional/required distinction driven by `SourceCatalog::auth_required`. Credential fields come from `redshank-core/src/domain/credentials.rs` — never re-derive them manually. |
